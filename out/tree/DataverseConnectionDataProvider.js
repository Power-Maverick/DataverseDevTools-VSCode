...
class DataverseConnectionDataProvider {
    ...
    getChildren(element) {
        if (element) {
            ...
            // Parent
            const results = Extensions_1.groupBy(this.connections, (c) => c.connectionType);
            let parentTree = [];
            Constants_1.environmentTypes.map((t) => {
                if (results[t]) {
                    parentTree.push(new DataverseConnectionTreeItem(t, undefined, vscode.TreeItemCollapsibleState.Collapsed, 1));
                }
            });
